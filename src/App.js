...
export default function RestApp(){
  ...
  // },[fetchData])
  useEffect(() => {
    const contryData = axios.get(`https://restcountries.com/v3.1/all`).then((res) =>{
  ...
  useEffect(()=>{
    ...
    a.map((reg_data)=>{
      if(regions.includes(reg_data.region)!=true){
        regions.push(reg_data.region)
        // console.log(regions.includes(reg_data.region))
      }
